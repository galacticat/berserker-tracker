...
def analyze_roll(dice):
    """Finds matching sets and returns a list of exploding sets to resolve individually."""
    if len(dice) < 2:
        return []
...
def roll_damage():
    ...
    # Check if THIS roll generated any NEW exploding sets
    new_exploding_sets = analyze_roll(dice)
    
    # Combine remaining pending sets with any new ones generated by this roll
    all_pending_sets = pending_sets + new_exploding_sets
    
    if len(all_pending_sets) > 0:
        # Take the next set off the queue to resolve immediately
...
def roll_str_loss():
    ...
    # Calculate weapon dice reduction if STR falls below requirement
    ...
    return jsonify({
        ...
        'message': "💥 YOU FELL UNCONSCIOUS! Berserking ends immediately." if is_unconscious else f"📉 Lost {str_lost} STR. STR remaining: {new_str}/{max_str}. Updated Adds: {new_adds}."
    })
